    ...
    private void OnLastWords(EntityUid uid, MobStateActionsComponent component, CritLastWordsEvent args)
    {
        ...
        _quickDialog.OpenDialog(actor.PlayerSession, Loc.GetString("action-name-crit-last-words"), "",
            (string lastWords) =>
            {
                ...
                _chat.TrySendInGameICMessage(uid, lastWords, SharedChatSystem.InGameICChatType.Whisper, SharedChatSystem.ChatTransmitRange.Normal, checkRadioPrefix: false, ignoreActionBlocker: true); // Corvax-Languages
                _host.ExecuteCommand(actor.PlayerSession, "ghost");
            });
